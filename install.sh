...
display_menu() {
    ...
    display_rathole_core_status
    echo
    echo -e "${GREEN}${BOLD} 1. Configure a new tunnel [IPv4/IPv6]${NC}"
    echo -e "${RED}${BOLD} 2. Tunnel management menu${NC}"
    echo -e "${CYAN}${BOLD} 3. Check tunnels status${NC}"
    echo -e "${YELLOW}${BOLD} 4. Optimize network & system limits${NC}"
    echo -e "${PINK}${BOLD} 5. Install rathole core${NC}"
    echo -e "${BLUE}${BOLD} 7. Change core [experimental]${NC}"
    echo -e "${BLUE}${BOLD} 8. Remove rathole core${NC}"
...
read_option() {
    echo -ne "${CYAN}${BOLD}Enter your choice [0-8]: ${NC}"
    read -p "" choice
    case $choice in
        1) configure_tunnel ;;
        2) tunnel_management ;;
